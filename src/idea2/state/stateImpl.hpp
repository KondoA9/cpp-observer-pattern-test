...
namespace Internal {
    ...
    template <typename T>
    class StateImpl final : public IState {
    public:
    ...
    private:
        StateImpl(size_t id, size_t groupId) : IState(id, groupId), m_value(getGroup().valuePtr()) {}

        StateGroup<T>& getGroup() const {
            return static_cast<StateGroup<T>&>(getGroupInterface());
        }
